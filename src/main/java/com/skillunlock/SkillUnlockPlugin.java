...
import com.skillunlock.ui.modern.ModernSkillUnlockPanel;
...
/**
 * Main plugin class for the Level up table
 * 
...
@PluginDescriptor(
	...
	tags = {"skill", "level", "unlock", "wiki"}
)
public class SkillUnlockPlugin extends Plugin
{
	@Inject
	private Client client;

	@Inject
	private SkillUnlockConfig config;
	...
	private ModernSkillUnlockPanel panel;
	...
	{
		log.info("Level up table started!");
		
		// Initialize services
		WikiClient wikiClient = new WikiClient(httpClient, gson);
		CacheManager cacheManager = new CacheManager();
		repository = new UnlockRepository(wikiClient, cacheManager);
		
		// Create panel
		panel = new ModernSkillUnlockPanel(client, config, repository, skillIconManager);
		
		// Create navigation button
		BufferedImage icon = loadPluginIcon();
		
		navButton = NavigationButton.builder()
			.tooltip("Level up table")
			.icon(icon)
			.panel(panel)
			.build();
	...
	{
		log.info("Level up table stopped!");
	...
	@Provides
	SkillUnlockConfig provideConfig(ConfigManager configManager)
	{
		return configManager.getConfig(SkillUnlockConfig.class);
	}
